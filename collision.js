...
function calculateIntersectionBetween(one, two) {
  var x = 0, y = 0
  ...
  if(x === 0 && y === 0) {
    if(intersects(one,two)) { // Should be able to get rid of this
      x = two.x - (one.x + one.width) 
      y = two.y - (one.y + one.height) 
    }
  }
...
function intersects(one, two) {
  ...
  if(bodyOne.type === RigidBody.TYPES.BOX && bodyTwo.type === bodyOne.type) 
    return boxIntersectsWithBox(one, two)
  else if(bodyOne.type === RigidBody.TYPES.CIRCLE && otherType === bodyOne.type) 
    return circleIntersectsWithCircle(one, two)
  else if(bodyOne.type === RigidBody.TYPES.CIRCLE) 
    return boxIntersectsWithCircle(one, two)
  else 
    return boxIntersectsWithCircle(one, two)
}
